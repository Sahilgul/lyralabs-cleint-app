...
2. **Plans** the steps using a tool catalog (Claude Sonnet primary, Gemini Flash for cheap calls).
...
| --------- | ------------- | ------------------------------------------------------------------------------------------- |
| `api`     | FastAPI 0.115 | Webhooks (Slack, Stripe), OAuth callbacks, admin REST API.                                  |
| `worker`  | Celery 5.4    | Run the LangGraph agent for each enqueued user request; resume on approval.                 |

**Admin UI (separate repo)**

The operator panel — a Vite + React 18 + React Router + Tailwind SPA — lives
in [`../lyralabs-admin-ui`](../lyralabs-admin-ui/) and ships as its own
Cloud Run service (Nginx-Alpine serving the static `dist/`). It calls this
API cross-origin; the FastAPI CORS allow-list (`ADMIN_BASE_URL`) must include
the deployed UI's origin.

**Backing services**

- Postgres 16 (tenants, users, integrations, jobs, audit, LangGraph checkpoints) — Supabase / Neon in prod.
- Redis (Celery broker + result backend) — Upstash in prod.
- Qdrant (per-tenant vector index) — Qdrant Cloud or self-hosted.
- Stripe (subscriptions + portal).
- LiteLLM router (Anthropic Claude Sonnet + Gemini Flash) — easy to swap providers.
...
- Node 20+ (only for the admin-ui — `make ui-dev` for the Vite dev server)
...
cp .env.example .env             # then fill in Slack/Google/GHL/Anthropic creds
...
| `ANTHROPIC_API_KEY`              | ✅       | Primary tier (Claude Sonnet 4.5).                                    |
| `GOOGLE_API_KEY`                 | ⚠️       | Cheap tier (Gemini 2.5 Flash). OpenAI is fallback.                   |
| `LLM_PRIMARY_MODEL`              |          | Default `anthropic/claude-sonnet-4-5`. Any LiteLLM-supported model.  |
| `LLM_CHEAP_MODEL`                |          | Default `gemini/gemini-2.5-flash`.                                   |
...
5. `gcloud run deploy` per service, blue/green via revisions
